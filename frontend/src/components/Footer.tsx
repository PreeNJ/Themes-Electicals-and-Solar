  ...
  return (
    <footer id="contact" className="section" style={{ borderTop: "1px solid var(--color-border)" }}>
      <div className="container" style={{ display: "grid", gap: "2rem", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))" }}>
        ...
        <div>
          <h4 style={{ marginBottom: "0.75rem", fontSize: "0.95rem" }}>Products</h4>
          <p style={{ color: "var(--color-text-muted)", fontSize: "0.9rem" }}>
            Solar Systems · Lighting · Water Pumps
            <br />
            Street Lights · Generators · Heat Pumps
          </p>
        </div>
      </div>

      <div className="container" style={{ marginTop: "2rem", paddingTop: "1.5rem", borderTop: "1px solid var(--color-border)" }}>
        <p style={{ color: "var(--color-text-muted)", fontSize: "0.85rem", textAlign: "center" }}>
          &copy; {year} Themes Electricals. All rights reserved.
        </p>
      </div>
    </footer>
  );
